...
local function sendReportToDiscord(source, target, reason)
	local message = table.concat({
		'**Reporter:** ' .. GetName(source),
		'**License:** ' .. tostring(GetIDFromSource(constants.IdentifierType, source) or 'unknown'),
		'**IP:** ' .. tostring(GetPlayerEndpoint(source) or 'unknown'),
		'',
		'**Player Reported:** ' .. GetName(target),
		'**License:** ' .. tostring(GetIDFromSource(constants.IdentifierType, target) or 'unknown'),
	...
	Server.sendDiscordWebhook('report', source, GetName(source), message, reportColor, function(ok, statusCode, body)
		if ok then
...
AddEventHandler('poodlechat:mute', function(player)
	...
	if id then
		local license = GetIDFromSource(constants.IdentifierType, id)
		if license then
...
AddEventHandler('playerJoining', function()
	...
	if Server.isDiscordKindEnabled('join') then
		Server.sendDiscordWebhook('join', source, GetName(source), 'connected to the server.', nil)
	end
end)
